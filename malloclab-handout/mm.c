...
// delete the node in the segregated free list to input ( if the pointer is in
// the list 2, after delete and insert again in the 5th list)
static void delete_node(void *ptr) {
  int index;
...
static void *coalesce(void *ptr) {
  ...
  if (prev_all == 1 && next_all == 0) {
    ...
    // update the info at the header and the footer of the new free block at the
    // pointer
...
// place the size in the appropriate block in the free list and decide to
// whether it is needed to split the block return the pointer to the new
// allocated block
static void *place(void *ptr, size_t asize) {
  size_t size = GET_SIZE(HDRP(ptr));
...
void *mm_malloc(size_t size) {
  ...
  while (index < LIST) {
    ...
    if ((index == LIST - 1) || (search <= 1 && free_lists[index] != NULL)) {
      ...
      // ignore the block with reallcation bit and find the smallest different
      // size block
